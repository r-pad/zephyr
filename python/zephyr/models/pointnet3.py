...
from .pointnet import PointNetCls, PointNetfeat
from .base import BaseModel

class PointNet3SSG(BaseModel):
    def __init__(self, input_dim,
                 args, num_class = 1
                 ):
        super(PointNet3SSG, self).__init__(args)
        ...
        self.model_channel = args.model_channel
        ...
        print("mask:", self.mask_channel, "xyz:", self.xyz_channel, "points:", self.points_channel, "model:", self.model_channel)

        '''Define the PointNet for object model'''
        self.pn_bottleneck = 16
        self.pn_object = PointNetfeat(
            input_dim = len(self.model_channel), global_feat=True,
            feature_transform = False, input_transform = False,
            mask_channel = self.mask_channel, pool = 'max',
            bottleneck_dim = self.pn_bottleneck
            )
        ...
        if self.no_coord:
            in_channel1 = len(self.points_channel) + self.pn_bottleneck
            in_channel2 = self.mlp1[-1]
            in_channel3 = self.mlp2[-1]
        else:
            in_channel1 = len(self.points_channel) + len(self.xyz_channel) + self.pn_bottleneck
    ...
    def forward(self, data):
        ...
        # Pass the model data into a PointNet first to get global information
        model_x = x[:, self.model_channel, :]
        # print("model_x", model_x.shape)
        model_global, _, _ = self.pn_object(model_x)
        # print("model_global", model_global.shape)
        model_global = model_global.unsqueeze(-1).repeat(1, 1, x.shape[2])
        # print("model_global after repeat", model_global.shape)
        # print("points", points.shape)
        points = torch.cat([points, model_global], 1)
        # print("points after cat", points.shape)
